          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div className="relative hidden xl:block">
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold text-slate-700 hover:bg-slate-50 transition-colors"
              >
              ...
              {isCategoriesDropdownOpen && (
                <div
                  ...
                  className="absolute left-0 top-full mt-2 w-64 bg-white rounded-2xl shadow-dropdown border border-slate-100 p-2 z-50 animate-fade-in"
                >
                  {MOCK_CATEGORIES.map((cat) => (
                    <Link
                      ...
                      className="flex items-center gap-3 p-2.5 rounded-xl hover:bg-brand-50 text-slate-700 hover:text-brand-700 transition-colors"
                    >
                  ...
                  <div className="pt-1.5 mt-1.5 border-t border-slate-100">
                    <Link
                      href="/categories"
                      onClick={() => setIsCategoriesDropdownOpen(false)}
                      className="block text-center text-xs font-bold text-brand-600 hover:underline py-1"
                    >
                      View All Categories →
                    </Link>
                  </div>
            ...
            <Link
              href="/wishlist"
              className="relative p-2.5 rounded-xl text-slate-600 hover:bg-slate-50 hover:text-rose-500 transition-colors"
              aria-label="Wishlist"
            >
                ...
                <div className="absolute right-0 top-full mt-2 w-56 bg-white rounded-2xl shadow-dropdown border border-slate-100 p-2 z-50 animate-fade-in">
                  {isAuthenticated && user ? (
                    ...
                    <div className="p-2 space-y-2">
                      <p className="text-xs text-slate-500 px-1">Welcome to Cartify</p>
                      <Link
                        href="/login"
                        onClick={() => setIsUserDropdownOpen(false)}
                        className="block w-full text-center py-2 bg-brand-600 hover:bg-brand-700 text-white rounded-xl text-xs font-semibold transition-colors"
                      >
                        Sign In
                      </Link>
                      <Link
                        href="/register"
                        onClick={() => setIsUserDropdownOpen(false)}
                        className="block w-full text-center py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl text-xs font-semibold transition-colors"
                      >
                        Create Account
                      </Link>
                    </div>
            ...
            <button
              ...
              className="flex items-center gap-2 px-3.5 py-2.5 rounded-xl bg-brand-600 hover:bg-brand-700 text-white shadow-sm hover:shadow transition-all"
